...
import { useAppProtectionPoliciesStore } from '../../store/appProtectionPoliciesStore';
import type { AppProtectionPolicyListItem } from '../../types/applications';
import '../../styles/workspace.css';

const columns: GridColDef<AppProtectionPolicyListItem>[] = [
  {
    field: 'displayName',
    headerName: 'Policy',
    flex: 2,
  ...
  { field: 'policyType', headerName: 'Type', width: 200 },
  ...
  return (
    <Toolbar style={{ gap: 8, padding: '8px 12px' }}>
      <QuickFilter defaultExpanded style={{ flex: 1 }}>
        <QuickFilterControl placeholder="Filter app protection policies..." size="small" style={{ minWidth: 200, maxWidth: 360 }} />
...
function DetailView() {
  const detail = useAppProtectionPoliciesStore((s) => s.detail);
  const isLoading = useAppProtectionPoliciesStore((s) => s.isLoadingDetail);
  const clearSelection = useAppProtectionPoliciesStore((s) => s.clearSelection);
      ...
      <div className="panel-body">
        <div className="fluent-detail-view">
          <div className="fluent-title-block">
            ...
            <span>{detail.policyType} · {detail.platform}</span>
          </div>
          ...
          <section className="fluent-section">
            <div className="fluent-section-header"><strong>Version requirements</strong></div>
            <div className="fluent-subsection">
              <div className="fluent-properties">
                <div className="fluent-property-row"><div className="fluent-property-label">Minimum app version</div><div className="fluent-property-value">{detail.minimumRequiredAppVersion || '—'}</div></div>
                <div className="fluent-property-row"><div className="fluent-property-label">Minimum OS version</div><div className="fluent-property-value">{detail.minimumRequiredOsVersion || '—'}</div></div>
              </div>
            </div>
          </section>
...
export function AppProtectionPoliciesWorkspace() {
  const items = useAppProtectionPoliciesStore((s) => s.items);
  const selectedId = useAppProtectionPoliciesStore((s) => s.selectedId);
  const isLoadingList = useAppProtectionPoliciesStore((s) => s.isLoadingList);
  const hasAttemptedLoad = useAppProtectionPoliciesStore((s) => s.hasAttemptedLoad);
  const error = useAppProtectionPoliciesStore((s) => s.error);
  const loadItems = useAppProtectionPoliciesStore((s) => s.loadItems);
  const selectItem = useAppProtectionPoliciesStore((s) => s.selectItem);
    ...
    <div className="workspace">
      <div className="workspace-toolbar">
        <div className="workspace-heading">
          <strong className="workspace-title">App Protection Policies</strong>
      ...
      <div className={`settings-columns${selectedId ? ' detail-active' : ''}`}>
        <div className="panel panel-list">
          ...
          <div style={{ width: '100%' }}>
            <DataGrid<AppProtectionPolicyListItem>
              rows={items}
              columns={columns}
              loading={isLoadingList}
              getRowId={(row) => row.id}
              rowSelectionModel={rowSelectionModel}
              onRowClick={(params: GridRowParams<AppProtectionPolicyListItem>) => void selectItem(params.row.id)}
              getRowHeight={() => 'auto'}
              showToolbar
              slots={{ toolbar: PolicyToolbar }}
