...
import type { DeviceConfigListItem } from '../../types/phase4';
import '../../styles/workspace.css';

const columns: GridColDef<DeviceConfigListItem>[] = [
  {
    field: 'displayName',
  ...
  { field: 'configurationType', headerName: 'Type', width: 180 },
  {
  ...
  return (
    <Toolbar style={{ gap: 8, padding: '8px 12px' }}>
      <QuickFilter defaultExpanded style={{ flex: 1 }}>
        <QuickFilterControl placeholder="Filter profiles..." size="small" style={{ minWidth: 200, maxWidth: 360 }} />
...
function DetailView() {
  const detail = useDeviceConfigStore((s) => s.detail);
  const isLoading = useDeviceConfigStore((s) => s.isLoadingDetail);
  const clearSelection = useDeviceConfigStore((s) => s.clearSelection);
...
export function DeviceConfigWorkspace() {
  const items = useDeviceConfigStore((s) => s.items);
  const selectedId = useDeviceConfigStore((s) => s.selectedId);
  const isLoadingList = useDeviceConfigStore((s) => s.isLoadingList);
  const hasAttemptedLoad = useDeviceConfigStore((s) => s.hasAttemptedLoad);
  const error = useDeviceConfigStore((s) => s.error);
  const loadItems = useDeviceConfigStore((s) => s.loadItems);
  const selectItem = useDeviceConfigStore((s) => s.selectItem);
    ...
    <div className="workspace">
      <div className="workspace-toolbar">
        <div className="workspace-heading">
          <strong className="workspace-title">Device Configurations</strong>
          <div className="workspace-stats"><span className="inline-stat">{items.length} profiles</span></div>
        </div>
      </div>
      {error && <div className="workspace-error">{error}</div>}
      <div className={`settings-columns${selectedId ? ' detail-active' : ''}`}>
        <div className="panel panel-list">
          <div className="panel-header"><strong>Profiles</strong><span>{items.length}</span></div>
          <div style={{ width: '100%' }}>
            <DataGrid<DeviceConfigListItem> rows={items} columns={columns} loading={isLoadingList}
              getRowId={(r) => r.id} rowSelectionModel={rowSelectionModel}
              onRowClick={(p: GridRowParams<DeviceConfigListItem>) => void selectItem(p.row.id)}
              getRowHeight={() => 'auto'} showToolbar slots={{ toolbar: ConfigToolbar }}
      ...
      <div className="workspace-footer"><span>{items.length} profiles</span>{isLoadingList && <span>Loading...</span>}</div>
    </div>
  );
}
