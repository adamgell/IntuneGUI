...
const { useEndpointSecurityStore } = await import('../endpointSecurityStore');

const initialState = useEndpointSecurityStore.getState();

beforeEach(() => {
  useEndpointSecurityStore.setState({ ...initialState });
  vi.clearAllMocks();
});

describe('endpointSecurityStore', () => {
  it('has correct initial state', () => {
    const state = useEndpointSecurityStore.getState();
  ...
  describe('loadItems', () => {
    it('loads items successfully', async () => {
      const mockItems = [{ id: '1', displayName: 'Security A' }, { id: '2', displayName: 'Security B' }];
      mockSendCommand.mockResolvedValueOnce(mockItems);

      await useEndpointSecurityStore.getState().loadItems();

      const state = useEndpointSecurityStore.getState();
      expect(mockSendCommand).toHaveBeenCalledWith('endpointSecurity.list');
    ...
    it('uses fallback error message for non-Error throws', async () => {
      ...
      expect(useEndpointSecurityStore.getState().error).toBe('Failed to load');
    });
  });

  describe('selectItem', () => {
    it('selects an item and loads detail', async () => {
      const mockDetail = { id: '1', displayName: 'Security A', settings: {} };
      mockSendCommand.mockResolvedValueOnce(mockDetail);

      await useEndpointSecurityStore.getState().selectItem('1');

      const state = useEndpointSecurityStore.getState();
      expect(mockSendCommand).toHaveBeenCalledWith('endpointSecurity.getDetail', { id: '1' });
    ...
    it('discards result if selection changed (stale guard)', async () => {
      const mockDetail = { id: '1', displayName: 'Security A' };
      mockSendCommand.mockImplementation(() => {
        useEndpointSecurityStore.setState({ selectedId: '2' });
  ...
  describe('clearSelection', () => {
    it('resets selection state', () => {
      useEndpointSecurityStore.setState({
        selectedId: '1',
        detail: { id: '1' } as any,
        isLoadingDetail: true,
      });

      useEndpointSecurityStore.getState().clearSelection();
